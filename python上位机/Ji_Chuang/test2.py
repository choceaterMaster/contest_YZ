...
if (rd.analoginstatus == 2):
    ...
    data_filt ,_,_= kalman_filter(rd.aidatach1, Q=1, R=1)
    fs = 100000
    y = np.fft.fft(data_filt)
    freq = list(np.linspace(0, fs, 2048))

    value = list(data_filt)  # the ch1 scope data as c_char_p(4086)  don't use rd.aidatach1.value
    plt.plot(value)
    plt.figure()
